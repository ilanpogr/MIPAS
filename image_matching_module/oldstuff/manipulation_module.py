...
def vintage_filter(tuple_image_path_and_name, index):
    im = ReadingUtils.reading_image_from_tuple_path_open_cv(tuple_image_path_and_name)
    ...
    WritingUtils.writing_vintage_to_path(tuple_image_path_and_name, index, vintage_im, "vintage")

#median filter
def median_filter (tuple_image_path_and_name):
    img = ReadingUtils.reading_image_from_tuple_path_open_cv(tuple_image_path_and_name)
    median = cv2.medianBlur(img,7)
    WritingUtils.writing_img_to_path(tuple_image_path_and_name, median, "median")

#roteting image.
def rotate(tuple_image_path_and_name,delta_given):
    rotate = 0
    delta = delta_given
    image = ReadingUtils.reading_image_from_tuple_path_using_image(tuple_image_path_and_name)
    for i in range(0,int(360 / delta)):
        rotated_image = image.rotate(rotate, fillcolor='white', expand=True)
        name = rotate
        rotate += delta
        WritingUtils.writing_rotate_to_path(tuple_image_path_and_name, name, "rotated", rotated_image)
...
def greyscale(tuple_image_path_and_name):
    image = ReadingUtils.reading_image_from_tuple_path_open_cv(tuple_image_path_and_name)
    gray = rgb2gray(image)
    WritingUtils.writing_img_to_path(tuple_image_path_and_name, gray, "greyscale")
...
def crop(tuple_image_path_and_name):
    ...
    WritingUtils.writing_img_to_path(tuple_image_path_and_name, crop_img, "cropped")
...
def flipped_image(tuple_image_path_and_name):
    image = ReadingUtils.reading_image_from_tuple_path_open_cv(tuple_image_path_and_name)
    flip_vertical = flipped_image_vertical(image)
    WritingUtils.writing_img_to_path_mirrored(tuple_image_path_and_name, flip_vertical, "mirroredVertical")
    flip_horizontal = flipped_image_horizontal(image)
    WritingUtils.writing_img_to_path_mirrored(tuple_image_path_and_name, flip_horizontal, "mirroredHorizontal")
    flip_ver_and_hori = flipped_image_horizontal_and_vertical(image)
    WritingUtils.writing_img_to_path_mirrored(tuple_image_path_and_name, flip_ver_and_hori, "mirroredVerAndHori")
...
def main_func_manipulation(source,images_list):
    ...
    images_list = ReadingUtils.get_images_names_in_folder(source)
    WritingUtils.create_folders_for_images(images_list)
    WritingUtils.create_sub_folders_in_pic_folder(images_list)
