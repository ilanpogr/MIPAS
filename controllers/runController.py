...
def download_products(signal_process, signal_status, signal_task):
    signal_task.emit("Download Products From Stores")
    user_stores = configUtils.get_property('store_name').split(',')
    download_products_for_all_stores(signal_process, signal_status, user_stores)
...
#
# def start_button_clicked(signal_process, signal_status):
#     # if state == 0:
